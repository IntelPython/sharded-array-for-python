...
class TestEWB:
    ...
    def test_add4(self):
        for dtyp in mpi_idtypes:
            n = 16
            a = sp.fromfunction(lambda i, j: i, (n, n), dtype=dtyp)
            b = sp.ones((n, n), dtyp)
            c = a + b
            a[:, :] = a[:, :] + c[:, :]
            r1 = sp.sum(a, [0, 1])
    ...
    def test_add_shifted1(self):
        for dtyp in mpi_idtypes:
            aa = sp.ones((16, 16), dtype=dtyp)
            bb = sp.ones((16, 16), dtype=dtyp)
            a = aa[0:8, 0:16]
            b = bb[5:13, 0:16]
            c = a + b + 1
            r1 = sp.sum(c, [0, 1])
    ...
    def test_add_shifted3(self):
        def doit():
            aa = sp.arange(0, 64, 1, dtype=sp.int32)
            bb = sp.arange(0, 64, 1, dtype=sp.int32)
            a = aa[0:8]
            b = bb[50:58]
            c = a + b + 1
            return sp.sum(c)
    ...
    def test_prod_het(self):
        a = sp.full([16, 16], 2, sp.float32)
        b = sp.full([16, 16], 2, sp.int32)
        c = a * b
        r = sp.sum(c, [0, 1])
        v = 16 * 16 * 2 * 2
        assert float(r) == v
    ...
    def test_pow(self):
        for dtyp in [sp.int32, sp.float32]:
            a = sp.full((6, 6), 3, dtype=dtyp)
            b = sp.full((6, 6), 2, dtype=dtyp)
            c = a**b
            r1 = sp.sum(c, [0, 1])
            v = 6 * 6 * 9
            assert float(r1) == v
    ...
    def test_add_typecast(self):
        ...
        for atype, btype, ctype in type_list:
            a = sp.arange(0, 8, 1, dtype=atype)
            b = sp.ones((8,), dtype=btype)
            c = a + b
    ...
    def test_reflected_sub(self):
        a = sp.full((4,), 4, dtype=sp.float64)
        b = 10.0
        c = b - a
        c2 = sp.to_numpy(c)
    ...
    def test_reflected_div(self):
        a = sp.full((4,), 2, dtype=sp.float64)
        b = 10.0
        c = b / a
        c2 = sp.to_numpy(c)
