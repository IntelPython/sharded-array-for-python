...
public:
    ...
    public:
        Copier()
            : _chunks(0), _curr_chunk(0), _curr_pos()
        {}
        Copier(const chunk_vec * chunks)
            : _chunks(chunks), _curr_chunk(_chunks->begin()), _curr_pos(_chunks->size() ? std::get<SLC>((*_curr_chunk)).begin() : NDSlice::iterator())
        {}
        ...
        Copier copy(T * buff, uint64_t capacity, uint64_t & ncopied) noexcept
        {
            ...
            while(ncopied < capacity) {
                ...
                uint64_t _nc;
                _curr_pos = _curr_pos.fill_buffer(buff + ncopied, capacity - ncopied, _nc,
