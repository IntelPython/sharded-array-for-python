...
inline xt::xstrided_slice_vector to_xt(const NDSlice &slice) {
  ...
  for (auto s : slice.slices())
    sv.push_back(xt::range(s._start, s._end, s._step));
  return sv;
}

using DPTensorBaseX = tensor_i;

template <typename T> class DPTensorX : public DPTensorBaseX {
...
public:
  ...
  DPTensorX(PVSlice &&slc, I &&ax, rank_type owner = NOOWNER)
      : _owner(owner),
        _slice(std::move(
            slc)), // static_cast<int>(owner==REPLICATED ? NOSPLIT : 0)),
  ...
  DPTensorX(const T &v, rank_type owner = getTransceiver()->rank())
      : _owner(owner),
        ...
        _xarray(std::make_shared<xt::xarray<T>>(1)), _replica(v) {
    *_xarray = v;
  }
  ...
  DPTensorX(const DPTensorX<O> &org, const NDSlice &slc,
      ...
      : _owner(owner), _slice(org._slice, slc),
        _lslice(to_xt(_slice.tile_slice())), _xarray(org._xarray) {
  ...
  DPTensorX(O &&org, PVSlice &&slc)
      : _owner(getTransceiver()->rank()), _slice(std::forward<PVSlice>(slc)),
        _lslice(to_xt(_slice.tile_slice())), _xarray() {
    _xarray = org;
  }

  ~DPTensorX() {}

  bool is_sliced() const { return _slice.is_sliced(); }
  ...
  virtual DTypeId dtype() const { return DTYPE<T>::value; }

  virtual const shape_type &shape() const { return _slice.shape(); }

  virtual int ndim() const { return _slice.ndims(); }

  virtual uint64_t size() const { return _slice.size(); }
  ...
  virtual bool __bool__() const {
    if (!is_replicated())
      throw(
          std::runtime_error("Cast to scalar bool: tensor is not replicated"));
  ...
  virtual double __float__() const {
    if (!is_replicated())
      throw(
          std::runtime_error("Cast to scalar float: tensor is not replicated"));
  ...
  virtual uint64_t __len__() const { return _slice.slice().dim(0).size(); }

  xt::xarray<T> &xarray() { return *_xarray.get(); }

  const xt::xarray<T> &xarray() const { return *_xarray.get(); }

  const PVSlice &slice() const { return _slice; }

  const xt::xstrided_slice_vector &lslice() const { return _lslice; }
  ...
  virtual int item_size() const { return sizeof(T); }
...
template <typename T> class operatorx {
public:
  ...
  static typename DPTensorX<T>::typed_ptr_type
  mk_tx(uint64_t rank, void *allocated, void *aligned, intptr_t offset,
  ...
  static DPTensorBaseX::ptr_type mk_tx_(const DPTensorX<T> &tx, X &&x) {
    return std::make_shared<DPTensorX<typename X::value_type>>(
        tx.shape(), std::forward<X>(x));
  }

  template <typename X>
  static DPTensorBaseX::ptr_type
  mk_tx_(const typename DPTensorX<T>::typed_ptr_type &tx, X &&x) {
    return std::make_shared<DPTensorX<typename X::value_type>>(
        tx->shape(), std::forward<X>(x));
  }
