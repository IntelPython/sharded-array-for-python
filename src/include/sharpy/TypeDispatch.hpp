...
// Recursiovely called to downcast array_i::ptr_type to typed shared_pointers.
// Downcasted array arg is removed from front of arg list and appended to its end.
// All other arguments are opaquely passed to the operation.
template<typename OpDispatch, typename... Ts>
auto TypeDispatch(const array_i::ptr_type & a_ptr, Ts&&... args)
{
...
// Root overload when no untyped arrays are in parameter pack.
template<typename OpDispatch, typename... Ts>
